...
test.describe('SSR rate-limit handling', () => {
	...
	test('changes page renders a retry overlay instead of a 500 when the API rate-limits', async ({
		page
	}) => {
		...
		// The rate-limit overlay must be visible with countdown text.
	...
	test('retry button re-issues the API request after the countdown expires', async ({ page }) => {
		...
		// The sim still answers 429, so the overlay must re-appear with a
